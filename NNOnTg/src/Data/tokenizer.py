...
class BPETokenizer(spm.SentencePieceProcessor):
    def __init__(self, vocab_size=None, learn=False):
        ...
        with open(path_to_data_file_config, 'r', encoding='utf-8') as config_data_file:
            ...
            self.path_to_file_with_raw_dialogs = "./data/row_data_2.json"
            self.path_to_file_with_raw_texts = "./data/books_dataset.json"
        ...
        if learn:
            ...
            self.load_tokenizer("./data/tokenizers/bpe_spm_tokenizer_ru.json")
    ...
    def train_bpe(self):
        ...
        print("Начинаем обучение BPE токенизатора...")
        ...
        spm.SentencePieceTrainer.train(
            ...
            model_type='bpe',  # Использовать BPE вместо unigram
            pad_id=0,
            unk_id=1,
            bos_id=2, 
            eos_id=3,
            user_defined_symbols=['<user>', '<bot>', '<eos>'],
            # Настройки для более агрессивных слияний
            split_digits=False,  # Не разбивать цифры
            split_by_unicode_script=False,  # Не разделять по скриптам
            split_by_whitespace=False,  # Сохранять пробелы как часть токенов
            # Более агрессивное слияние
            max_sentencepiece_length=16,  # Увеличить максимальную длину токена
    ...
    def pre_tokenize(self, text: str) -> list:
        return text.lower()
    ...
    def tokenize_dialog(self, dialog: list[str]) -> list[list[int]]:
        ...
        for phrase in dialog:
            phrase = self.pre_tokenize(phrase)
    ...
    def detokenize_text(self, token_ids: list) -> str:
        ...
        # Фильтрация специальных токенов которые не умеет обрабатывать SentencePiece
        filtered_ids = []
